...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setProduct(prod);
        if (prod.images && prod.images.length > 0) setMainImage(`http://localhost:5000${prod.images[0]}`);
        return;
      } catch (err) {
        console.log("Backend not found, checking assets...");
      }

      // Check assets if backend not found
  ...
  const imagesArray = product.images?.length ? product.images.map(img => `http://localhost:5000${img}`) : product.image || [];
 ...
 const handleAddToCart = () => {
  ...
  addToCart(productForCart, selectedSize, quantity);
    ...
    <div>
      <div className="w-[90%] sm:w-[85%] lg:w-[82%] mx-auto my-10 flex flex-col lg:flex-row gap-8 lg:gap-10">
        ...
        <div className="flex-1 relative flex justify-center items-center order-1 lg:order-2">
          <img src={mainImage} alt={product.name} className="rounded-xl w-full max-h-[500px] sm:max-h-[600px] object-contain bg-white"/>
